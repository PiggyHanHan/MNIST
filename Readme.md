...
**核心特点**：
- 数据集：EMNIST Balanced（约 28 万张训练图，47 类）
...
## 📂 数据准备
- **EMNIST Balanced**：通过 `torchvision.datasets.EMNIST(split='balanced')` 自动下载，训练集约 28 万张，测试集约 2.8 万张，图片为 28×28 灰度图。
- 类别说明：共有 47 个类别，包含数字 0-9 和合并后的大小写字母（例如 'C' 与 'c' 合并为一类）。具体映射可通过训练脚本生成的 `emnist_classes.pkl` 查看。

---

## 🧠 模型定义
沿用简单 CNN 结构（输入 1 通道，输出 47 类）：
- 卷积层：`Conv2d(1,32,3) + ReLU + MaxPool2d(2)` → `Conv2d(32,64,3) + ReLU + MaxPool2d(2)`
- 全连接层：`Linear(64*7*7, 128) + ReLU` → `Linear(128, 47)`
...
- 批量大小：64
- 训练轮次：12
- 测试集准确率：约 **95%~97%**（随随机种子略有波动）

训练完成后会生成两个文件：
- `emnist_cnn_balanced.pth`：模型权重
- `emnist_classes.pkl`：类别列表（索引 → 字符）
...
### 2. 图片背景不是纯黑/白会影响结果吗？
- 预处理中的 OTSU 二值化和极性归一化会强制转换为黑底白字，因此背景颜色不影响。但若图片对比度极低，二值化可能失败，建议保证数字清晰。

### 3. 类别映射从哪里来？
- 训练时自动生成 `emnist_classes.pkl`，预测时必须保证该文件存在且与模型匹配。
